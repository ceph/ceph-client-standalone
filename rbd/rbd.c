			  ...
			  u64 *ver)
{
	...
	ceph_osdc_set_request_linger(&dev->client->osdc, req, linger);
			  ...
			  u64 ver)
{
	...
	ret = ceph_osdc_register_notify_msg(osdc, rbd_watch_cb,
					    (void *)dev, &watcher);
	if (ret < 0)
		goto done;

	ops[0].watch.ver = cpu_to_le64(ver);
	ops[0].watch.cookie = cpu_to_le64(watcher->cookie);
	ops[0].watch.flag = 1;
		          ...
		          const char *obj)
{
	...
	ret = ceph_osdc_register_notify_msg(osdc, rbd_notify_cb,
					    (void *)&info, &watcher);
	if (ret < 0)
		goto done;
	...
	ops[0].watch.ver = 1;
	ops[0].watch.flag = 1;
	ops[0].watch.cookie = watcher->cookie;
	...
	ret = rbd_req_sync_op(dev, NULL,
			       ...
			       CEPH_OSD_FLAG_WRITE | CEPH_OSD_FLAG_ONDISK,
			       ops,
			       1, obj, 0, 0, NULL, 0, NULL);
	if (ret < 0)
		goto done_notify;

	ret = ceph_osdc_wait_watch_notify(watcher, CEPH_OSD_TIMEOUT_DEFAULT);
	dout("ceph_osdc_wait_watch_notify returned %d\n", ret);
